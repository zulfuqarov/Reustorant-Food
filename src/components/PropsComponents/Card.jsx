...
const Card = () => {
    ...
    const start = (context.currentPage - 1) * 6
    const end = start + 6
    if (context.loading) {
        return (
        ...
        return (
            <div className='grid grid-cols-3 gap-4 pt-[40px] max-[991px]:grid-cols-2 max-[768px]:grid-cols-1'>
                {
                    context.product.length > 0 ?
                        context.product.slice(start, end).map((oneMap, index) => (
                        ...
                        context.allProduct.slice(start, end).map((oneMap, index) => (
                            <div key={index} className="w-full mx-auto max-w-[344px] sm:max-w-[280px] h-auto p-4">
                                <div className="relative bg-[url(/rectangle-8.svg)] bg-cover rounded-lg overflow-hidden">
                                    <div className="w-full h-auto">
                                        <img className="w-full h-auto rounded-t-lg" alt="Mask group" src={SpaggetiSvg} />
